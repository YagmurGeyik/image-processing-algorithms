...
def cv2_benzeri_esikleme(goruntu, esik_degeri=127):
    binary = [[0 for _ in range(len(goruntu[0]))] for _ in range(len(goruntu))]
    for i in range(len(goruntu)):
        for j in range(len(goruntu[0])):
            piksel = goruntu[i][j]
            if isinstance(piksel, list):  # RGB görüntü ise: ortalama değeri al
                piksel = sum(piksel) // len(piksel)
            if piksel > esik_degeri:
                binary[i][j] = 255  # Beyaz yap
    return esik_degeri, binary
